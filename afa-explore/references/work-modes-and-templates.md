...
### 2.3 模式三：竞争格局分析 (Competitive Landscape Analysis)

**触发条件**：进入新市场前、竞品发布重大新品、定价策略调整。**输入要求**：核心竞品列表（数量按当前格局复杂度确定）、分析维度（流量、定价、产品、内容）。**执行流程**：(1) 确认竞品范围 → (2) 收集各竞品的公开数据 → (3) 分析流量来源结构 → (4) 分析定价策略与 SKU 结构 → (5) 分析广告素材与核心卖点 → (6) 分析用户评价（重点关注低分与中性评论中的信号） → (7) 绘制竞品全景图 → (8) 识别市场空白。**标准输出**：《竞品全景图》、《市场空白与机会报告》。
